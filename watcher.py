...
class Watcher():
    def __init__(self, instances):
        # In here we log init watcher class
        ...
        # We call load playlist function
        playlist = self.load_playlist()

        # And we create a new list with instances count
        videos_playlist = list(np.repeat(playlist, instances))

        # Then we watch all videos from playlist
        if len(videos_playlist) > 0:
            self.watch_video(videos_playlist)
        # And IF no videos found
        else:
